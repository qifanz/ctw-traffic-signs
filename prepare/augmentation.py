...
with open(settings.TRAIN) as f:
    ...
    for i in range(len(json_lines)):
        ...
        for j in range(len(annotations)):
            sentence = annotations[j]
            new_sentence = []
            for word in sentence:
                if len(word['attributes']) == 0:
                    has_traffic_signs = True
                    new_sentence.append(word)
            annotations[j] = new_sentence
        if has_traffic_signs:
            list_filtered.append(annot)
            image = os.path.join (settings.TRAINVAL_IMAGE_DIR, id+'.jpg')
            image_tar = os.path.join(settings.TRAINVAL_AUGMENTED_DIR, id+'.jpg')
            shutil.copy2(image,image_tar)
...
with open(settings.VAL) as f:
    f1 = open(settings.VAL_PROCESSED, "w+")
    list_filtered = []
    json_lines = f.readlines()
    for i in range(len(json_lines)):
        annot = json.loads(json_lines[i])
